# db_manager.py (النسخة النهائية مع دالة جلب آخر تاريخ)
import sqlite3
import pandas as pd
import os
import logging
...
class DatabaseManager:
    ...
    def get_latest_session_date(self) -> Optional[str]:
        """
        يسترجع أحدث تاريخ جلسة مسجل في قاعدة البيانات للمقارنة.
        هذه الدالة لا تستخدم الكاش لأننا نحتاجها أن تكون حديثة دائماً.
        """
        logger.info("Fetching latest session date from database.")
        try:
            with sqlite3.connect(self.db_filename) as conn:
                # هذا الاستعلام يفترض أن التواريخ مخزنة كنص "DD/MM/YYYY"
                # ويقوم بترتيبها بشكل صحيح للعثور على الأحدث
                query = f"""
                SELECT "{C.SESSION_DATE_COLUMN_NAME}" 
                FROM "{C.TABLE_NAME}"
                ORDER BY 
                    SUBSTR("{C.SESSION_DATE_COLUMN_NAME}", 7, 4) DESC, -- Year
                    SUBSTR("{C.SESSION_DATE_COLUMN_NAME}", 4, 2) DESC, -- Month
                    SUBSTR("{C.SESSION_DATE_COLUMN_NAME}", 1, 2) DESC  -- Day
                LIMIT 1;
                """
                cursor = conn.cursor()
                ...
                if result:
                    ...
                    # هذا طبيعي إذا كانت قاعدة البيانات فارغة في أول تشغيل
                    logger.info("No session date found in DB (table might be empty).")
                    return None
        except sqlite3.Error as e:
