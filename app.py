# app.py (النسخة النهائية مع تنسيق أرقام موحد)
...
from utils import prepare_arabic_text, load_css, format_currency # <-- تم إضافة الدالة الجديدة هنا
...
def main():
            ...
            if not data_df.empty and "البيانات الأولية" not in last_update:
                ...
                for session_date_str in unique_dates:
                    try:
                        ...
                        purchase_info = ""
                        if day_ar == "الأحد": purchase_info = prepare_arabic_text("(يتم شراؤه يوم الخميس السابق)")
                        elif day_ar == "الاثنين": purchase_info = prepare_arabic_text("(يتم شراؤه يوم الأحد السابق)")
                        ...
                        if session_date_str != unique_dates[-1]: st.markdown("<hr style='border-color: #495057; margin: 10px 0 15px 0;'>", unsafe_allow_html=True)
                    except (ValueError, TypeError):
                        continue
            else:
    ...
    with top_col2:
        with st.container(border=True):
            ...
            if "البيانات الأولية" in last_update: st.warning(prepare_arabic_text("قاعدة البيانات فارغة. قم بتحديث البيانات."), icon="⏳")
            else:
                try:
                    last_update_dt = datetime.strptime(last_update.replace(prepare_arabic_text("بتاريخ "), ""), "%d-%m-%Y")
                    if (datetime.now(pytz.timezone("Africa/Cairo")).date() - last_update_dt.date()).days > 0:
    ...
    with col_form_main:
        with st.container(border=True):
            ...
            investment_amount_main = st.number_input(prepare_arabic_text("المبلغ المستهدف في نهاية المدة (القيمة الإسمية)"), min_value=25000.0, value=25000.0, step=25000.0)
            ...
            def get_yield_for_tenor(tenor):
                if not data_df.empty:
                    ...
                    if not yield_row.empty: return yield_row[C.YIELD_COLUMN_NAME].iloc[0]
                return None

            formatted_options = []
            for tenor in options:
                yield_val = get_yield_for_tenor(tenor)
                if yield_val is not None: formatted_options.append(f"{tenor} {prepare_arabic_text('يوم')} - ({yield_val:.3f}%)")
                else: formatted_options.append(f"{tenor} {prepare_arabic_text('يوم')}")
    ...
    if calculate_button_main and selected_tenor_main is not None:
        try:
            if data_df.empty:
                with results_placeholder_main.container(border=True): st.error(prepare_arabic_text("لا توجد بيانات للعائد. يرجى تحديث البيانات أولاً."))
            else:
                ...
                if yield_rate is not None:
                    ...
                    with results_placeholder_main.container(border=True):
                        ...
                        # --- START OF MODIFICATION FOR TASK 5 ---
                        ...
                        # --- END OF MODIFICATION FOR TASK 5 ---
                    ...
                    with results_placeholder_main.container(border=True): st.error(prepare_arabic_text("لم يتم العثور على عائد للأجل المحدد."))
        except Exception as e:
            with results_placeholder_main.container(border=True): st.error(prepare_arabic_text(f"حدث خطأ غير متوقع أثناء الحساب: {e}"), icon="🚨")
    else:
        with results_placeholder_main.container(border=True): st.info(prepare_arabic_text("✨ ملخص استثمارك سيظهر هنا بتصميم أنيق بعد الضغط على زر الحساب."))
    ...
    with col_secondary_form:
        with st.container(border=True):
            ...
            face_value_secondary = st.number_input(prepare_arabic_text("القيمة الإسمية للإذن"), min_value=25000.0, value=25000.0, step=25000.0, key="secondary_face_value")
            ...
            with secondary_results_placeholder.container(border=True):
                ...
                if results.get("error"):
                    ...
                    with st.expander(prepare_arabic_text("عرض تفاصيل الحساب")):
                        ...
                        if results["gross_profit"] <= 0: st.info(prepare_arabic_text("لا توجد ضريبة على الخسائر الرأسمالية."), icon="ℹ️")
                    # --- END OF MODIFICATION FOR TASK 5 ---
        except Exception as e:
            with secondary_results_placeholder.container(border=True): st.error(prepare_arabic_text(f"حدث خطأ غير متوقع أثناء الحساب: {e}"), icon="🚨")
    else:
        with secondary_results_placeholder.container(border=True): st.info(prepare_arabic_text("✨ أدخل بيانات البيع في النموذج لتحليل قرارك."))
